<html>
  ...
  <head>
    <title> customer Signup | Toy Rentals </title>
  </head>
  <?php session_start(); ?>
...
if (!empty($_FILES["uploadedimage"]["name"])) {
    ...
    if(move_uploaded_file($temp_name, $target_path)) {
      ...
      //  $query0 = "UPDATE cars SET car_img = '$target_path' WHERE ";
        ...
        $query = "INSERT into toys(toy_name,toy_nameplate,toy_img,price_per_day,owner_name,owner_contact,owner_location,toy_availability) VALUES('" . $car_name . "','" . $car_nameplate . "','".$target_path."','" . $ac_price . "','" . $non_ac_price . "','" . $ac_price_per_day . "','" . $non_ac_price_per_day . "','" . $car_availability ."')";
        $success = $conn->query($query);
...
$query1 = "SELECT toy_id from toys where toy_nameplate = '$car_nameplate'";
...
$query2 = "INSERT into clienttoys(toy_id,client_username) values('" . $car_id ."','" . $_SESSION['login_client'] . "')";
$success2 = $conn->query($query2);

if (!$success){ ?>
    <div class="container">
	<div class="jumbotron" style="text-align: center;">
        Toy with the same number already exists!
        <?php echo $conn->error; ?>
        <br><br>
        <a href="entertoy.php" class="btn btn-default"> Go Back </a>
</div>
<?php	
}
else {
    header("location: entertoy.php"); //Redirecting 
}
