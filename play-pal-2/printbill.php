...
$id = $_GET["id"];
$distance = NULL;
$distance_or_days = $conn->real_escape_string((int)$_POST['distance_or_days']);
$fare = $conn->real_escape_string((int)$_POST['hid_fare']);
$total_amount = $distance_or_days * $fare;
$car_return_date = date('Y-m-d');
$return_status = "R";
$login_customer = $_SESSION['login_customer'];

$sql0 = "SELECT rc.id, rc.rent_end_date, rc.charge_type, rc.rent_start_date, c.toy_name, c.toy_nameplate FROM rentedtoys rc, toys c WHERE id = '$id' AND c.toy_id = rc.toy_id";
...
if(mysqli_num_rows($result0) > 0) {
    while($row0 = mysqli_fetch_assoc($result0)){
            ...
            $car_name = $row0["toy_name"];
            $car_nameplate = $row0["toy_nameplate"];
...
if($charge_type == "days"){
    ...
    $sql1 = "UPDATE rentedtoys SET toy_return_date='$car_return_date', no_of_days='$no_of_days', total_amount='$total_amount', return_status='$return_status' WHERE id = '$id' ";
} else {
    $distance = $distance_or_days;
    $sql1 = "UPDATE rentedtoys SET toy_return_date='$car_return_date', distance='$distance', no_of_days='$duration', total_amount='$total_amount', return_status='$return_status' WHERE id = '$id' ";
}

$result1 = $conn->query($sql1);

if ($result1){
     $sql2 = "UPDATE toys c, deliverypartner d, rentedtoys rc SET c.toy_availability='yes', d.dp_availability='yes' 
     WHERE rc.toy_id=c.toy_id AND rc.dp_id=d.dp_id AND rc.customer_username = '$login_customer' AND rc.dp_id = '$id'";
...
?>
    ...
    <div class="container">
        <div class="jumbotron">
            <h1 class="text-center" style="color: green;"><span class="glyphicon glyphicon-ok-circle"></span> Toy Returned</h1>
        </div>
    </div>
    <br>

    <h2 class="text-center"> Thank you for visiting Toy Rentals! We wish you have a safe ride. </h2>
    ...
    <div class="container">
        ...
        <div class="box">
            ...
            <div class="col-md-10" style="float: none; margin: 0 auto; ">
                <h4> <strong>Toy Name: </strong> <?php echo $car_name;?></h4>
                <br>
                <h4> <strong>Unique Number:</strong> <?php echo $car_nameplate; ?></h4>
                <br>
            ...
            ?></h4>
                ...
                <br>
                <h4> <strong>Toy Return Date: </strong> <?php echo $car_return_date; ?> </h4>
                <br>
                <?php if($charge_type == "days"){?>
                    <h4> <strong>Number of days:</strong> <?php echo $distance_or_days; ?>day(s)</h4>
                <?php } else { ?>
                    <h4> <strong>Distance Travelled:</strong> <?php echo $distance_or_days; ?>days(s)</h4>
                <?php } ?>
                <br>
                <?php
                    if($extra_days > 0){
...
<footer class="site-footer">
        <div class="container">
            ...
            <div class="row">
                <div class="col-sm-6">
                    <h5>© <?php echo date("Y"); ?> Toy Rentals</h5>
                </div>
            </div>
        </div>
    </footer>
